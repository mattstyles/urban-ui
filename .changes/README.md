...
- Trains, not packages: `core` (everything under packages/, lockstep),
  `labs` (labs/, its own 0.x line), and one train per CLI binary
  (e.g. `urban`). Graduation names two trains: `core: minor`, `labs: major`.
